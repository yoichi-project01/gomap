...
import { updateSpot } from "@/lib/client/spots"
...
function pathFromCoverUrl(url: string): string {
  const marker = "/covers/"
  const idx = url.indexOf(marker)
  return idx >= 0 ? url.slice(idx + marker.length) : ""
}

export default function EditSpotForm({ spot }: { spot: Spot }) {
  const router = useRouter()
  const [title, setTitle] = useState(spot.name)
  const [description, setDescription] = useState(spot.description ?? "")
  const [category, setCategory] = useState<string | null>(spot.category ?? null)
  const [searchOpen, setSearchOpen] = useState(false)
  const [cover, setCover] = useState<{ url: string; path: string } | null>(
    spot.coverImageUrl ? { url: spot.coverImageUrl, path: pathFromCoverUrl(spot.coverImageUrl) } : null
  )
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white font-sans pb-24 overflow-y-auto [&::-webkit-scrollbar]:hidden">
      <header className="sticky top-0 z-50 flex items-center justify-between px-4 pt-10 pb-4 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-sm border-b border-zinc-200 dark:border-zinc-800">
        ...
        <h1 className="text-sm font-bold text-zinc-700 dark:text-zinc-300">スポットを編集</h1>
        <button
          onClick={handleSave}
          disabled={isSaving}
