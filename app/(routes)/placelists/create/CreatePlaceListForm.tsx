  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white font-sans pb-24 overflow-y-auto [&::-webkit-scrollbar]:hidden">
      <header className="sticky top-0 z-50 flex items-center justify-between px-4 pt-10 pb-4 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-sm border-b border-zinc-200 dark:border-zinc-800">
        <button
          onClick={handleBack}
          className="w-10 h-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center hover:bg-zinc-200 dark:hover:bg-zinc-700 transition"
        >
          <ChevronLeft className="w-6 h-6 text-zinc-900 dark:text-white" />
        </button>
        <h1 className="text-sm font-bold text-zinc-700 dark:text-zinc-300">新しいプレイスリストを作成</h1>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      <main className="px-4 mt-8 max-w-md md:max-w-2xl lg:max-w-4xl mx-auto flex flex-col gap-8">
        ...
        <div>
          <h2 className="text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-3">公開設定</h2>
        ...
        <div className="mt-2">
          ...
          <div className="flex flex-col gap-3 mb-6">
            {selectedSpots.length === 0 ? (
              <div className="text-xs text-zinc-500 text-center py-8 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-2xl">
                まだスポットが追加されていません
              </div>
            ) : (
              selectedSpots.map((spot, index) => (
                <div
                  key={spot.id}
                  className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-900 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-zinc-600 dark:text-zinc-400 text-sm font-bold">
                      {index + 1}
                    </div>
                    <div>
                      <h3 className="font-bold text-sm text-zinc-900 dark:text-white">{spot.name}</h3>
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white py-3 rounded-full font-bold text-xs transition"
            >
