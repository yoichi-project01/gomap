...
function NavRow({
  label, sub, href, onClick, danger = false, disabled = false,
}: {
  label: string; sub?: string; href?: string; onClick?: () => void; danger?: boolean; disabled?: boolean
}) {
...
export default function MyPageClient({ user, stats }: { user: MyPageUser; stats: MyPageStats }) {
  ...
  const [defaultPref, setDefaultPref] = useLocalStorageState<string>("gomap:defaultPref", "大阪府")
  const [defaultZoom, setDefaultZoom] = useLocalStorageState<string>("gomap:defaultZoom", "14")
  const [mapStyle, setMapStyle]       = useLocalStorageState<string>("gomap:mapStyle", "標準")

  const [spotsPublic, setSpotsPublic]     = useLocalStorageState<boolean>("gomap:spotsPublic", true)
  const [profilePublic, setProfilePublic] = useLocalStorageState<boolean>("gomap:profilePublic", true)

  const [pushNotif, setPushNotif]   = useLocalStorageState<boolean>("gomap:pushNotif", true)
