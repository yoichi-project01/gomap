      ...
      {placeLists.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {placeLists.map((placeList) => (
            <li key={placeList.id} className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl p-4">
              <div className="flex items-start justify-between gap-3">
                <Link href={`/placelists/${placeList.id}`} className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{placeList.name}</p>
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">
                    スポット {placeList.spotsCount}件
                  </p>
                </Link>
                <button
              ...
              <p className="text-xs text-zinc-300 dark:text-zinc-600 mt-2">いいね日: {placeList.likedAt}</p>
            </li>
          ))}
        </ul>
      )}
