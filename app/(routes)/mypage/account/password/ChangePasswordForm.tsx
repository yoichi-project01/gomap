...
function PasswordInput({
  id, name, label, autoComplete, minLength,
}: {
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          minLength={minLength}
          required
          className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 px-3 py-2 pr-10 text-sm outline-none focus:border-zinc-500"
        />
        <button
          type="button"
          onClick={() => setShow((v) => !v)}
          aria-label={show ? "パスワードを隠す" : "パスワードを表示"}
          className="absolute inset-y-0 right-0 px-3 flex items-center text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition"
        >
