...
export default function LocationSearchModal({ onClose, onSelect }: Props) {
  ...
  return (
    ...
    >
      <div className="sticky top-0 flex items-center gap-2 px-4 pt-10 pb-3 bg-black/95 border-b border-zinc-800">
        ...
        <button
          ...
          onClick={onClose}
          aria-label="閉じる"
          className="w-9 h-9 rounded-full hover:bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white transition"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 [&::-webkit-scrollbar]:hidden">
        ...
        {status === "error" && (
          <p className="text-xs text-red-400 py-8 text-center">{errorMessage}</p>
        )}
        {!hasMinLength && (
